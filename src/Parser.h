...
class NetlistParser{
    private:
        ...
        GateType string_to_gate(string &str){
            ...
            else if(upper == "OUTPUT") return GateType::OUTPUT;
            else if(upper == "AND") return GateType::AND;
            else if(upper == "OR") return GateType::OR;
            else if(upper == "NOT") return GateType::NOT;
        ...
        Node* get_or_create_node(string &name, GateType type, uint64_t delay = 1){
            ...
            Node* node = pool.allocate(name, type);
    ...
    public:
        ...
        bool load_file(const string& filepath){
            ...
            while(getline(file, line)){
                ...
                if(!(ss >> first_token) || first_token[0] == '#') continue;
        ...
        void set_inputs(const string& name, bool val){
            ...
            if(it != node_map.end() && it->second->type == GateType::INPUT){
                ...
                cerr<<"Error: Unknown input pin "<<name<<endl<<endl;
            }
        }
